...
Main.apiCall = function (targetPath) {
	...
	if(Main.getSessionID() == ''){
		...
		if (utilities.validateString(targetPath) == true ) {
			...
			var count =12;	
			if(targetPath.indexOf('section/') > -1){ // suppose its a viewall list page
				count =24;
			}
			try {
				Yup.ajax({
...
Main.apicallResponse= function(result,targetPath){	
	...
	if(result.response.info){		
			if(result.response.info.pageType == 'details'){
				...
				else if(result.response.info.attributes && result.response.info.attributes.contentType && result.response.info.attributes.contentType == 'channel'){
					responseArray.pageType= 'details';
	...
	// if(targetPath == firstMenu){
		...
		for(var i=0; i<systemConfigs.menus.length ; i++ ){
			...
			if(systemConfigs.menus[i].code != '' && systemConfigs.menus[i].code != 'packages' && systemConfigs.menus[i].code != 'package' && systemConfigs.menus[i].code != 'pricing' && systemConfigs.menus[i].code != 'tvguide' && systemConfigs.menus[i].code != 'schedule' && systemConfigs.menus[i].code != 'search'){
				// if(systemConfigs.menus[i].code == 'my_library'){
				// 	if(appConfig.enableMyLibrary == true)
	...
	// Tv guide 
	else if(isTvguide==true){
		Main.getTvguideChannelsList(responseArray);
		return;
	}
	else{
		//view
		if(presentPagedetails.targetPath =='guide' || presentPagedetails.targetPath=='tvguide') {responseArray.isComingTvguide=true;}
	...
	if(presentPagedetails.pageType == 'list'){
		if(presentPagedetails.isMenuPathforGrid == true){
		...
		success: function (data) {
			...
			if(result.status == true && result.response.length>0){				
				...
				if(window.window > 1280){
					Yup('.searchSuggestions').animate({height : 70 },200);
				}				
				else{
...
Main.processNext = function () {
	...
	if( view == 'search'){
		...
		Yup("#mainContent").html('');
		Yup("#mainContent").html(Util.signInHtml());
		Main.HideLoading();	
...
Main.getTvguideChannelsList=function(responseArray){
	Yup.ajax({
		...
		success: function (data) {
			...
			if(result.response.data.length > 0){
				responseArray.sections[0]={
					'sectionInfo':{
						'name':result.response.tiltle
					},
					'sectionControls':{'dataLength':result.response.data.length},
					'sectionData':{'data':result.response.data}
				}
				responseArray.listTitle=result.response.tiltle;
				view = responseArray.view;
				presentPagedetails = responseArray;
				Yup("#mainContent").html(Util.getContentHTML(presentPagedetails));
				Yup("#mainContent").show();
